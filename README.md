...
The MiniWob web task is represented by a simplified Markov Decision Process (MDP) (see following image). The nodes represent the states, while the arrows represent the actions that should be performed in the appropriate state. Every state-action-state transition has an assigned reward value. The maximum reward can be +1, while the maximum negativ reward is -1.:
